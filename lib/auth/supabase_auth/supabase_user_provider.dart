...
class ProyectoDAMGuardiasSupabaseUser extends BaseAuthUser {
  ProyectoDAMGuardiasSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> proyectoDAMGuardiasSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ProyectoDAMGuardiasSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
